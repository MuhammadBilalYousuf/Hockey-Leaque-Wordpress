...
<div class="sp-template sp-template-event-blocks">
    <div class="sp-table-wrapper">
        <table data-lsfs-live="<?php echo $id; ?>" class="sp-event-blocks sp-data-table<?php if ( $paginated ) { ?> sp-paginated-table<?php } ?>" data-sp-rows="<?php echo $rows; ?>">
            ...
            <tbody>
                ...
                foreach ( $data as $event ):
                    ...
                    ?>
                    <tr data-live-event="<?php echo $event->ID; ?>" class="sp-row sp-post<?php echo ( $i % 2 == 0 ? ' alternate' : '' ); ?>" itemscope itemtype="http://schema.org/SportsEvent">
                        <td>
                            ...
                            <h5 class="sp-event-results data-live-results">
                                <?php echo sp_add_link( '<span class="sp-result">' . implode( '</span> - <span class="sp-result">', apply_filters( 'sportspress_event_blocks_team_result_or_time', sp_get_main_results_or_time( $event ), $event->ID ) ) . '</span>' . lsfs_get_live_status( $event->ID ), $permalink, $link_events ); ?>
                            </h5>
